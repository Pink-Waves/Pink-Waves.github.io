...
export default function ControlledAccordions() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/api/messaging/favorite/'.concat(token), {
          method: 'GET',
  ...
  const clickChange = (message) => (event) => {
  ...
  const favoritesDisplay = Array.from(favoritesData).map(message => {
    return (
      <Accordion key={message.id} expanded={expanded === message.id} onChange={handleChange(message.id)} className={classes.coloring}>
        ...
        >
          <FormControlLabel
            ...
            control={<Checkbox defaultChecked={message.favorite} onChange={clickChange(message)} checkedIcon = {<Favorite/>} icon = {<FavoriteBorder/>}/>}
             />
          ...
          <Typography className={classes.heading}>From: {message.sender_username}</Typography>
  ...
  const EmptyFavorites = () => {
    return (
      <div className={classes.emptyList}>
        You don't have any favorited fletters yet!
      </div>
    )
  }
  ...
  const checkAll = () => {
    ...
    if(parent.checked == true) {
      Array.from(favoritesData).map(message => {
  ...
  return (
    ...
    <div className={classes.root}>
        <h1 className = {classes.justMargin}>Favorites</h1>
          ...
          <List style={{maxHeight: '50%', overflow: 'auto'}}>
            <div>{Array.from(favoritesData).length===0 ? EmptyFavorites() : null}</div>
            { favoritesDisplay }
          </List>
    </div>
    </>
